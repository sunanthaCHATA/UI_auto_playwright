...
test('Dashboard - Validate Filter Functionality on Newly Created Column', async ({ page }) => {
  ...
  // code to check filter functionality on the added custom column
  await dashboardPage.dashboardTile.hover();
  await dashboardPage.dashboardTileFilterOption.click();

  // code to validate column data in newly created column
  const dashboardTileContainer = page.locator(`.dashboard-tile-response-container`);
  const customColumnHeader = dashboardTileContainer.locator(`.tabulator-col-title-holder:has-text("${columnName}")`).locator("..");
  const customMoneylineIndex = await customColumnHeader.locator("..").getAttribute("tabulator-field");
  
  if (!customMoneylineIndex) {
    throw new Error(`Custom column field index not found for column: ${columnName}`);
  }

  // Get the filter value from the first row
  const firstRowCustomColumn = dashboardTileContainer.locator(`.tabulator-row:first-of-type div[tabulator-field="${customMoneylineIndex}"]`);
  const filterValueText = await firstRowCustomColumn.textContent();
  const filterValue = filterValueText?.trim() ?? "";

  // code to fill filter field with the value
  // The filter input for the custom column is in the header section
  const filterInput = dashboardTileContainer.locator(`.tabulator-col[tabulator-field="${customMoneylineIndex}"] .tabulator-header-filter input`);
  await filterInput.fill(filterValue);

  // code to send enter key to apply filter
  await filterInput.press('Enter');
  await page.waitForTimeout(5000);

  // code to validate filtered data in response table for first 5 rows
  const rowCount = await dashboardTileContainer.locator(".tabulator-row").count();

  for (let i = 0; i < rowCount && i < 5; i++) {
    const filteredDataText = await dashboardTileContainer.locator(`.tabulator-row:nth-of-type(${i + 1}) div[tabulator-field="${customMoneylineIndex}"]`).textContent();
    const filteredData = filteredDataText?.trim() ?? "";
    
    if (filteredData === filterValue) {
      console.log(`Filtered data is validated in row ${i + 1} with value: ${filteredData}`);
    } else {
      throw new Error(
        `Filtered data is NOT validated in row ${i + 1}. Expected value: ${filterValue}, but found: ${filteredData}`
      );
    }
  }
