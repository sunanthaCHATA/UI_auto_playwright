...
test('Dashboard - Validate custom Column functionality for single value response queryon Dashboard', async ({ page }) => {
...
await page.waitForTimeout(15000);
  ...
  if (await dashboardPage.createNewDashboardBtn.isVisible()) {
    await dashboardPage.createNewDashboardBtn.click();
  } else {
    await dashboardPage.createFirstDashboardBtn.click();
  }
  await dashboardPage.dashboardNameInput.fill(dashboardName);
  await dashboardPage.createDashboardBtn.click();
  await page.waitForTimeout(5000);

  await dashboardPage.dashboardRunQueryBtn.fill('total goals');
  ...
  await dashboardPage.variableColumnOption('Total Goals').click();
  ...
  // code to wait for the custom column to be added on dashboard and assert the column name
  await page.waitForTimeout(15000);
  await dashboardPage.dashboardColumnSelector(columnName).waitFor({ state: 'visible' , timeout: 15000});
