...
test('Dashboard - Validate Add Nested Column functionality on Dashboard', async ({ page }) => {
  ...
  const nestedColumnName = `Nsd_col_${shortTs}`;
  ...
  // Code to add nested custom column on dashboard using the previously added custom column as part of the formula and validate the column addition
  await dashboardPage.addColumnBtn.click();
  await dashboardPage.addcustomColumnOption.click();
  ...
  await page.keyboard.press('Control+A');
  await page.keyboard.press('Delete');
  await dashboardPage.customColumnNameInput.fill(nestedColumnName); 

  // code to fill formula for the custom column
  await dashboardPage.variableColumnOption(columnName).click();
  await dashboardPage.plusOperator.click();  
  await dashboardPage.variableCustomNumberOption.click();
  await dashboardPage.customnumberInput(1).fill("50");

  // Code to Save the custom column and verify the column is added on dashboard
  await page.waitForTimeout(5000);
  await dashboardPage.customColumnSaveBtn.click();

  // Assertion to verify the nested column is added on Dashboard
  await page.waitForTimeout(15000);
  await dashboardPage.dashboardColumnSelector(nestedColumnName).waitFor({ state: 'visible' , timeout: 15000});





  // code to validate the  data in the nested custom column is correct based on the formula used
  ...
  const nestedCustomColumnField = await page.getByRole('columnheader', { name: nestedColumnName, exact: true }).getAttribute('tabulator-field');
  ...
  for (let i = 0; i < rowsToValidate; i++) {
    ...
    const nestedValue = parseFloat(await rows.nth(i).locator(`.tabulator-cell[tabulator-field="${nestedCustomColumnField}"]`).textContent() || "0");
    const expectedValue = customValue + 50;

    expect(nestedValue).toBeCloseTo(expectedValue, 2);
    console.log(`Row ${i + 1}: Custom Column = ${customValue}, Nested Custom Column = ${nestedValue}, Expected Nested Custom Column = ${expectedValue}`);
  }
