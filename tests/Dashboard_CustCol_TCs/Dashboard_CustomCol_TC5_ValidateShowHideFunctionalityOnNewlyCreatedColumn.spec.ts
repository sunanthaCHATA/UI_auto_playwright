...
test('Dashboard - Validate Show/Hide Functionality on Newly Created Column', async ({ page }) => {
  ...
  // code to uncheck the newly created column from show/hide column option and validate the column is hidden from dashboard tile
  await dashboardPage.dashboardTile.hover();
  await dashboardPage.dashbaordTileShowHideColumnOption.click();
  await page.locator(`//div[contains(text(),'${columnName}')]/ancestor::div[contains(@class,'react-autoql-list-item')]//input[@type='checkbox']`).uncheck();
  await dashboardPage.dashbaordApplyButton.click();
  await page.waitForTimeout(5000);

  // Validate the column is hidden from dashboard tile
  const dashboardTileContainer = page.locator(`.dashboard-tile-response-container`);
  await dashboardTileContainer.locator(`.tabulator-col-title:has-text("${columnName}")`).waitFor({ state: 'hidden', timeout: 10000 });
  console.log("Validated the column is hidden from dashboard tile.");



  // Code to select All Column Options
  await dashboardPage.dashboardTile.hover();
  await dashboardPage.dashbaordTileShowHideColumnOption.click();
  await dashboardPage.dashboardShowHideSelectAllOption.check();
  await dashboardPage.dashbaordApplyButton.click();
  ...
  // Validate the column is displayed again on dashboard tile
  await dashboardTileContainer.locator(`.tabulator-col-title:has-text("${columnName}")`).waitFor({ state: 'visible', timeout: 10000 });
  console.log("Validated the column is displayed again on dashboard tile after selecting show column option.");
