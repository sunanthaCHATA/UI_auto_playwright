...
test('Dashboard - Validate Download CSV Functionality on Newly Created Column', async ({ page }) => {
  ...
  // Code to download the dashboard data in CSV format and validate the downloaded file
  await dashboardPage.dashboardTile.hover();
  
  // Initiate download and wait for notification messages
  const csvFilePath = await dashboardHelper.downloadCSV();

  // Validate the CSV file content
  await dashboardHelper.validateCSV(csvFilePath, columnName);
