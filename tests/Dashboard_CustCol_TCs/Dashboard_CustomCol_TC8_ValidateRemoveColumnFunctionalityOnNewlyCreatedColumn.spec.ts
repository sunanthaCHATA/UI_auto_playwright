...
test('Dashboard - Validate Remove Column Functionality on Newly Created Column', async ({ page }) => {
  ...
  // Code to validate the Remove Column option functionality an column should get removed from dashboard and should not exist
  await dashboardPage.removecolumnOption.click();
  await page.waitForTimeout(2000);
  const isColumnVisible = await targetColumn.isVisible().catch(() => false);
  expect(isColumnVisible).toBeFalsy();
  console.log("Custom Column is successfully removed from Dashboard and not visible on UI"); 




  // Code to add the same column again and then validate the presence of column
  await dashboardPage.addColumnBtn.click();
  await page.locator(`//*[@class='context-menu-list']//*[text()='${columnName}']`).click();

  // Code to validate the column is added again on dashboard
  await dashboardPage.dashboardColumnSelector(columnName).waitFor({ state: 'visible', timeout: 10000 });
  console.log("Custom Column is added again on Dashboard with name: " + columnName);
