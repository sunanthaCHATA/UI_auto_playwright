...
import { DashboardPage, AxisType, DashboardPage_Locators } from '../../utils/pages/DashboardPage';
...
test('Dashboard - Validate New Columns on Charts', async ({ page }) => {
  ...
  // Define column selector XPaths for different chart types
  const columnSelectorXpath = "//*[@data-test='selectable-list']//*[contains(text(), 'ColumnName')]";
  
  // Validate  all Column Chart's
  await dashboardHelper.validateColumnInChart(DashboardPage_Locators.dashboardColumnChart, AxisType.Y_AXIS,columnSelectorXpath,columnName,true );
  await dashboardHelper.validateColumnInChart(DashboardPage_Locators.dashboardBarChart, AxisType.X_AXIS,columnSelectorXpath,columnName,true );
  await dashboardHelper.validateColumnInChart(DashboardPage_Locators.dashboardLineChart,AxisType.Y_AXIS,columnSelectorXpath,columnName, true );
  await dashboardHelper.validateColumnInChart(DashboardPage_Locators.dashboardStackedBarChart, AxisType.X_AXIS, columnSelectorXpath,columnName,true );
  await dashboardHelper.validateColumnInChart(DashboardPage_Locators.dashboardStackedColumnChart,AxisType.Y_AXIS,columnSelectorXpath,columnName, true );
  await dashboardHelper.validateColumnInChart(DashboardPage_Locators.dashboardColumnLineComboChart,AxisType.Y_AXIS,columnSelectorXpath, columnName,true );
  await dashboardHelper.validateColumnInChart(DashboardPage_Locators.dashboardHistogram,AxisType.BOTH,DashboardPage_Locators.dashboardChartsHistogramScatterplotColumnSelector,columnName,true );
  await dashboardHelper.validateColumnInChart(DashboardPage_Locators.dashboardScatterplot,AxisType.BOTH,DashboardPage_Locators.dashboardChartsHistogramScatterplotColumnSelector,columnName,true);
